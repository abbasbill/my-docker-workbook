```
id: 09_gcp_taxi_scheduled_forEach
namespace: zoomcamp
description: |
  Refactored to use ForEach over year-month and taxi-type combinations with Subflow.
...
inputs:
  - id: year_months
    type: ARRAY
    displayName: Year-Month combinations (YYYY-MM format)
    defaults: ["2024-01", "2024-02", "2024-03"]
  
  - id: taxi_types
    type: ARRAY
    displayName: Taxi types to process
...
tasks:
  - id: generate_combinations
    ...
    values: |
      {% set combinations = [] %}
      {% for year_month in inputs.year_months %}
        {% for taxi in inputs.taxi_types %}
          {% set _ = combinations.append({'year_month': year_month, 'taxi': taxi}) %}
        {% endfor %}
      {% endfor %}
      {{ combinations }}
    tasks:
      - id: process_combination
        type: io.kestra.plugin.core.flow.Subflow
        namespace: zoomcamp
        flowId: 09_gcp_taxi_processing_subflow
        inputs:
          taxi: "{{taskrun.value.taxi}}"
          year_month: "{{taskrun.value.year_month}}"
        wait: true
        transmitFailed: true
...
triggers:
  - id: monthly_schedule
    ...
    cron: "0 9 1 * *"
    inputs:
      year_months: ["{{trigger.date | date('yyyy-MM')}}"]
      taxi_types: ["yellow", "green"]


id: 09_gcp_taxi_processing_subflow
namespace: zoomcamp
description: Subflow to process a single taxi type for a specific year-month

inputs:
  - id: taxi
    type: STRING
    displayName: Taxi type (yellow or green)
  
  - id: year_month
    type: STRING
    displayName: Year-Month in YYYY-MM format

variables:
  file: "{{inputs.taxi}}_tripdata_{{inputs.year_month}}.csv"
  gcs_file: "gs://{{kv('GCP_BUCKET_NAME')}}/{{vars.file}}"
  table: "{{kv('GCP_DATASET')}}.{{inputs.taxi}}_tripdata_{{inputs.year_month | replace('-', '_')}}"
...
tasks:
  ...
  - id: upload_to_gcs
    ...
    from: "{{outputs.extract.outputFiles[inputs.taxi ~ '_tripdata_' ~ inputs.year_month ~ '.csv']}}"
    to: "{{render(vars.gcs_file)}}"

  - id: "create_main_table"
    type: io.kestra.plugin.gcp.bigquery.Query
    sql: |
      CREATE TABLE IF NOT EXISTS `{{kv('GCP_PROJECT_ID')}}.{{kv('GCP_DATASET')}}.{{inputs.taxi}}_tripdata`
      (
  ...
  - id: "merge_data"
    type: io.kestra.plugin.gcp.bigquery.Query
    sql: |
      MERGE INTO `{{kv('GCP_PROJECT_ID')}}.{{kv('GCP_DATASET')}}.{{inputs.taxi}}_tripdata` T
